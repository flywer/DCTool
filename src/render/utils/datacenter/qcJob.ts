...
export const buildQcJson = async (formModel: QcFormModelType) => {

    const project = await find_by_project_id(formModel.projectId)

    formModel.tableName = formModel.tableName.toLowerCase()

    const tableName = `di_${project?.tableAbbr}_${formModel.tableName}_temp_ods`

    paramsJson.modelJson = paramsJson.modelJson.replace(/sourceTable/g, `${tableName}`).replace(/targetTable/g, `${tableName}`)

    paramsJson = JSON.parse(updateSjkUUID(removeIds(paramsJson)))

    paramsJson.name = `qc_${project.projectAbbr}_${formModel.tableName}`
    ...
    paramsJson.dataDevBizVo.sparkSqlDtoList[0].sql = `INSERT INTO ` + `${tableName}` + ' SELECT * FROM ' + `${tableName}` + ' WHERE 1 = 0'
    paramsJson.dataDevBizVo.sparkSqlDtoList[0].sourceTable = [`${tableName}`]
    paramsJson.dataDevBizVo.sparkSqlDtoList[0].targetTable = tableName
