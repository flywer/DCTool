...
const tableDataInit = async () => {
  ...
  // 获取缓存数据
  const cacheItems = await fetch_job_table_item_cache('actionData2ThemeBase')
  // 若存在缓存数据，则先获取缓存数据
  ...
  const actionRkJobs: Workflow[] = allActionRkJobs.filter(job => job.procName.includes('rk_sztk_'))
  ...
  await save_job_table_item_cache('actionData2ThemeBase', tableDataRef.value)
  ...
  saveModalFormRef.value?.validate(async errors => {
    if (!errors) {
      ...
      const model = new RkJobSaveModel({
        name: `rk_sztk_${saveModalFormModel.value.tableName}`,
        projectId: projectId,
        personId: saveModalFormModel.value.personId,
        sourceDataSourceId: '6',
        sourceTableName: `df_sztk_${saveModalFormModel.value.tableName}_dm`,
