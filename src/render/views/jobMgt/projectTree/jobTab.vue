    ...
    if (projectAbbr !== '') {
      ...
      for (const v of dataXJobs) {
        ...
        const job: Job = {
          ...
          lastExecTime: getJobType(v.jobDesc) == JobType.cj ? (schedJob?.incStartTime || '--') : (v?.triggerLastTime || '--'),
      ...
      newWorkflowJobs = newWorkflowJobs.filter(job => ![JobType.odstj, JobType.odstjbf,JobType.odsjl].includes(job.type))
      // endregion
